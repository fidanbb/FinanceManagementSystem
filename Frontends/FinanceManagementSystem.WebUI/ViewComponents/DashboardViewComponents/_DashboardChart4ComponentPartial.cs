...
namespace FinanceManagementSystem.WebUI.ViewComponents.DashboardViewComponents
{
    public class _DashboardChart4ComponentPartial:ViewComponent
    {
        ...
        public _DashboardChart4ComponentPartial(IHttpClientFactory httpClientFactory, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            if (token is not null)
            {
                ...
                var responseMessage = await client.GetAsync($"Transactions/GetLastFiveMonthsExpense/{userId}");

                if (responseMessage.IsSuccessStatusCode)
                {
                    var jsonData = await responseMessage.Content.ReadAsStringAsync();

                    var values = JsonConvert.DeserializeObject<List<GetLastFiveMonthExpenseDto>>(jsonData);
